...
function updateProfilePhoto() {
  ...
  if (topAvatar) {
    ...
    topAvatar.innerHTML =
      `<img src="${escapeHtml(state.photoUrl)}"
       alt="Profile">`;
...
function updateProfileUI() {
  ...
  if (state.verified) {
    ...
    if (verificationContent) {
      ...
      verificationContent.innerHTML = `
        <p style="color:#20d889;">
          ...
          All available features are unlocked.
        </p>
      `;
    ...
    if (verificationContent) {
      ...
      verificationContent.innerHTML = `
        <p>
          Join both Official Channel and Group
          to verify your account and unlock
          withdrawal and referral benefits.
        </p>

        <a
          href="${APP_CONFIG.channelLink}"
          target="_blank"
...
function setupLinks() {
  ...
  if (channel) {
    channel.href =
      APP_CONFIG.channelLink;
  }


  if (group) {
    group.href =
      APP_CONFIG.groupLink;
  }
}
  ...
  if (shareButton) {
    ...
    shareButton.addEventListener(
      ...
      () => {
        ...
        const text =
          encodeURIComponent(
            "Join BS TASK ZONE and complete tasks to earn rewards!"
          );
  ...
  button.addEventListener(
    ...
    () => {
      ...
      if (!state.verified) {
        ...
        showMessage(
          "🔒 Please verify your membership in both Channel and Group first."
        );
      ...
      showMessage(
        `Withdraw request: ${state.withdrawMethod} ৳${amount}`
      );
...
function updateSpinButton() {
  ...
  if (
    ...
    button.textContent =
      `🔒 Need ${APP_CONFIG.spinCost} Points`;
...
async function performSpin() {
  ...
  if (
    Number(state.referralPoints) <
    APP_CONFIG.spinCost
  ) {

    showMessage(
      `You need ${APP_CONFIG.spinCost} Referral Points.`
    );
