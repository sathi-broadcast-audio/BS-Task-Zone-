...
function renderReferralList() {
  ...
  container.innerHTML = state.referrals
    .map(referral => {
      const name = referral.username ? `@${referral.username}` : referral.name || "Telegram User";
...
function updateProfileUI() {
  ...
  if (state.verified) {
    ...
    if (verificationContent) {
      verificationContent.innerHTML = `
        <p>Join the Official Channel and verify your account to unlock withdrawal and referral benefits.</p>
...
async function verifyChannel() {
  ...
  try {
    ...
    if (state.verified) {
      ...
      showMessage("🔴 Please join the Official Channel first.");
    }
  } catch (error) {
    showMessage("Verification failed. Try again.");
  }
...
async function openTask(taskId) {
  try {
    ...
    showMessage("Unable to open task.");
  }
}
...
/* =========================================
   Withdraw Logic (Connected to Supabase Table)
========================================= */
...
function setupWithdraw() {
  ...
  button.addEventListener("click", async () => {
    ...
    if (!state.verified) {
      showMessage("🔒 Please verify your Official Channel first.");
      return;
    }
    ...
    if (!/^01[0-9]{9}$/.test(number)) {
      showMessage("Enter a valid 11-digit account number.");
      return;
    }

    if (!amount || amount <= 0) {
      showMessage("Enter a valid withdraw amount.");
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        showMessage("✅ Withdraw request submitted successfully!");
        state.balance = data.newBalance;
        updateBalanceUI();
        document.getElementById("withdraw-amount").value = "";
        document.getElementById("withdraw-number").value = "";
      } else {
        showMessage(data.message || "Withdraw failed.");
      }
    } catch (error) {
...
async function performSpin() {
  ...
  if (Number(state.referralPoints) < APP_CONFIG.spinCost) {
    showMessage(`You need ${APP_CONFIG.spinCost} Referral Points.`);
    return;
  }
  ...
  try {
    ...
    showMessage(`🎉 Congratulations! You won ${winningReward} points from Spin!`);
    
    // Here you can also add API call to update spin reward points if needed
  } catch (error) {
    showMessage("Spin failed. Try again.");
  } finally {
